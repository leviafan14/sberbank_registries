...
zip_path="/home/k/Downloads/" #Откуда загружаем архив с реестром
...
patch_script='/home/k/reesters_python_17_2/script/sberbank_registries.py' # путь к скрипту
#Описание функций проекта
#-----------------------------------------------
# Функция эмитации платежа за интернет 
def inet_pay_test(abonent,pay_sum,trans_num):
    return 0
...
def archive():
    ...
    try:
        ...
        return r_list
          ...
          messagebox.showerror('Ошибка', 'Не удалось распаковать реестр')
          return  False
#----------------------------------        
    ...
    if(str(r_name_list[1])=='02'):
        ...
        while reester_len>0: #Пока длина реестра больше 0, на каждой итерации цикла reester_len уменьшается на 1
          ...
          if (pay_sum==140 or pay_sum==280 or pay_sum==420 or pay_sum==560 or pay_sum==720 or pay_sum==860):#Проверка, если сумма платежа предположительно за ТВ
              ...
              #action_pay=inet_pay_test(abonent,pay_sum,trans_num)#Запуск функции эмитации платежа за интернет
          print ('Результат: '+str(action_pay))
          type_pay(abonent, pay_sum, trans_num, date_transaction, date_check, action_pay)
          result_row='№:'+str(trans_num).strip()+' дата платежа:'+str(date_transaction)+' л/с:'+str(abonent)+' сумма:'+str(pay_sum)+' дата получения:'+str(date_transaction)+state_pay 
...
lbl_path_script=Label(window, text="Путь к скрипту: "+patch_script)
